use std::{marker::PhantomData, ffi::{CString, CStr}, mem::MaybeUninit};

use hxcfe_sys::{HXCFE_IMGLDR, HXCFE_FSMNG, hxcfe_initFsManager, hxcfe_selectFS, hxcfe_mountImage, hxcfe_deinitFsManager, hxcfe_openDir, hxcfe_readDir, HXCFE_FSENTRY, FSENTRY_, hxcfe_closeDir};

use crate::{Hxcfe, img::Img};
...
impl<'hfe> FileSystemManager<'hfe> {
	...
	pub fn open_dir(&self, folder: &str) -> Result<DirHandler, i32> {
		...
		if dirhandle > 0 {
			Ok(DirHandler{dirhandle, fs_manager: self})
		} else {
			Err(dirhandle)
		}
	}
