...
use hxcfe_sys::{HXCFE,HXCFE_IMGLDR, HXCFE_FLOPPY};
...
use crate::{Hxcfe, img_loaders::ImgLoader};
...
impl Img {
	...
	pub fn nb_tracks(&self) -> i32 {
		let res = unsafe{hxcfe_getNumberOfTrack (self.hxcfe.as_ref().unwrap().handler, self.floppydisk,)};
		res
	}
	...
	pub fn nb_sides(&self) -> i32 {
		let res = unsafe{hxcfe_getNumberOfSide (self.hxcfe.as_ref().unwrap().handler, self.floppydisk,)};
		res
	}

	pub fn size(&self) -> i32 {
		let mut nbofsector = 0;
		let size = unsafe{hxcfe_getFloppySize (self.hxcfe.as_ref().unwrap().handler, self.floppydisk, &mut nbofsector)};
		size
	}

	pub fn nb_sectors(&self) -> i32 {
		let mut nbofsector = 0;
		let _ = unsafe{hxcfe_getFloppySize (self.hxcfe.as_ref().unwrap().handler, self.floppydisk, &mut nbofsector)};
		nbofsector
	}
}
...
impl<'img> Interface< 'img> {
	pub fn name(&self) -> &str {
		let res = unsafe{hxcfe_getFloppyInterfaceModeName(self.img.hxcfe.as_ref().unwrap().handler, self.ifmode)};
		unsafe{CStr::from_ptr(res)}.to_str().unwrap()
	}

	pub fn desc(&self) -> &str {
		let res = unsafe{hxcfe_getFloppyInterfaceModeDesc(self.img.hxcfe.as_ref().unwrap().handler, self.ifmode)};
		unsafe{CStr::from_ptr(res)}.to_str().unwrap()
	}
}