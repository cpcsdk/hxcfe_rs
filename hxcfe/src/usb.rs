use crate::{DriveId, Hxcfe, Img, HxcfeError, InterfaceModeId, TrackId};
...
impl UsbHxcfe {
    ...
    pub fn eject_floppy(&self) -> Result<(), HxcfeError> {
        let ret = unsafe {
            hxcfe_sys::usbhxcfe::libusbhxcfe_ejectFloppy(**self.hxcfe, self.handler)
        };
    ...
    pub fn get_double_step(&self) -> bool {
        unsafe {
            hxcfe_sys::usbhxcfe::libusbhxcfe_getDoubleStep(**self.hxcfe, self.handler) != 0
        }
    }
