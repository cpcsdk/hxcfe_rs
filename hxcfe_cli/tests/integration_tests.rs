...
fn run_cli(args: &[&str], work_dir: &Path) -> Result<std::process::Output, Box<dyn std::error::Error>> {
...
fn test_fs_operations_fat720() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 1..=6 {
        let filename = format!("text0{}.txt", i);
        let output = env.run_cli(&["--finput", image_path, "--getfile", &format!("/{}", filename)])?;
    ...
    if md5_path.exists() {
        ...
        assert_eq!(generated_md5_output.trim(), expected_md5_content.trim(),
            "MD5 output doesn't match md5.txt exactly.\nGenerated:\n{}\nExpected:\n{}", 
            generated_md5_output, expected_md5_content);
    }
...
fn test_format_conversion_fat() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (input_file, _) in &formats {
        ...
        assert!(output.status.success(), 
            "Reverse conversion of {} failed", input_file);
        
        let test_path = env.work_dir.join(&test_output);
        assert!(test_path.exists(), 
            "Reverse conversion file {} was not created", test_output);
    }
    ...
    for (input_file, _) in &formats {
        ...
        if test_hash == ref_hash {
            ...
            println!("✗ {} roundtrip differs: ref={}, test={}", 
                input_file, ref_hash, test_hash);
        }
    }
    
    // All 18 conversions should be identical
    assert_eq!(identical_count, 18, 
        "Expected 18 identical roundtrip conversions, got {}", identical_count);
...
fn test_format_conversion_amiga() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(identical_count, 8, 
        "Expected 8 identical roundtrip conversions, got {}", identical_count);
    
    // Additional test: ADZ→ADF with MD5 verification (matching convert_amiga.sh)
    let output = env.run_cli(&[
        "--finput", "ADOS_1760kB.adz",
        "--foutput", "C001.ADF",
        "--conv", "RAW_LOADER"
    ])?;
    ...
    assert_eq!(c001_hash, expected_hash,
        "ADZ→ADF MD5 mismatch: expected {}, got {}", expected_hash, c001_hash);
...
fn test_additional_md5_conversions() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let conversions = vec![
        ("hbd.vdk", "C001.IMG", "RAW_LOADER", "4292a3cb2f6a6466a39583271334f8ce"),
        ("disk1.86f", "C002.IMG", "RAW_LOADER", "73bec3e0512925c955c706a9d742967d"),
        ("Acorn_Horizon.adf", "C003.IMG", "RAW_LOADER", "05f58f4fa3b96017d4ea0c1c80f24c0b"),
        ("EvilIn11.ssd", "C004.IMG", "RAW_LOADER", "cee7770810257f2063b5765ff24c2525"),
        ("adc-cpm.td0", "C005.IMG", "RAW_LOADER", "ac12f9cfcd68ff364598507f7ca1503e"),
        ("dos33_with_adt.do", "C006.DO", "APPLE2_DO", "7c350e5da3672bca4abbdbe67fdaf14a"),
        ("Apple_DOS_3_3_January_1983.do", "C007.DO", "APPLE2_DO", "b13de32fd7a97d817744bf2dd71d5479"),
        ("Apple_DOS_3_3_January_1983.nib", "C008.DO", "APPLE2_DO", "b13de32fd7a97d817744bf2dd71d5479"),
        ("apridisk.dsk", "C009.IMG", "RAW_LOADER", "f35a690248f7afebc5180b4e81cceb88"),
    ];
    ...
    assert!(success_count > 0, 
        "At least some MD5 conversions should pass, got {}", success_count);
...
fn test_comprehensive_suite() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(success_count, total_tests, 
        "Expected {} successful tests, got {}", total_tests, success_count);
